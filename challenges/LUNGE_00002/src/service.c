...
void register_book(program_state *s, char *section, char *name, char *data) {
  ...
  if (!section) err(1, "invalid section");

  if (!name) err(1, "invalid name");

  if (!data) err(1, "invalid data");
  ...
  while (bk != NULL) {
    if (0 == strlen(bk->section)) {
      ...
      if (!bk->next) err(1, "unable to allocate memory");
 ...
 * to leverage the marker "|" to modify memory in a fashion that can be
 * leveraged gain execution control.
 */
int normalize_path(char *path) {
  char *p;
  char *q;
    ...
    if (*p == TRAVERSAL) {
      if (current_delim == p - 1 || p == path) {
        if (p[1] == TRAVERSAL && (p[2] == DELIM || p[2] == '\0')) {
          ...
          if (depth > 2) goto fail;

          if (previous_delim == NULL) goto done;
          ...
          if (current_delim != NULL) {
            if (current_delim >= end_of_path) goto done;

            strlcpy(current_delim, (char *)(end_of_path - current_delim), len);
            continue;
          } else {
            if (p >= end_of_path) goto done;

            strlcpy(p, (char *)(end_of_path - p), len);
            continue;
          }
        } else if (p[1] != '\0') {
          if (current_delim != NULL) p = current_delim;
...
int skip_search(size_t *skip_dict, char *needle, size_t needle_len,
...
int cmd_search(program_state *s, char *args) {
  ...
  while (cur_book != NULL) {
    if (skip_search((size_t *)&skip_dict, args, scan_len, cur_book->data,
                    strlen(cur_book->data)) == 1) {
...
int cmd_ch_sec(program_state *s, char *args) {
  ...
  if (normalize_path(args) != 0) return -1;

  sub_section_len = strlen(args);

  if (args[0] != DELIM) return 0;
...
int cmd_compress(program_state *s, char *args) {
  ...
  if (0 == strlen(args)) return -1;

  if (0 == strlen(s->current_dir)) return -1;
...
int cmd_get(program_state *s, char *args) {
  ...
  if (NULL == s->current_dir) return -1;
...
int cmd_list(program_state *s, char *args) {
  ...
  while (cur_book != NULL) {
    if (0 == strlen(cur_book->section)) break;

    if (0 ==
        strncmp(cur_book->section, s->current_dir, strlen(s->current_dir))) {
      if (strlen(cur_book->section) - strlen(s->current_dir) > 0) {
        transmit_all(STDOUT, cur_book->section + strlen(s->current_dir),
                     strlen(cur_book->section) - strlen(s->current_dir));
        transmit_str(STDOUT, ":");
      }
...
int process(program_state *s) {
  ...
  while (1) {
    ...
    if (strlen(s->input_buf) == 0) continue;

    name = strtok(s->input_buf, " ");
    args = strtok(NULL, "\x00");

    if (strlen(name) >= COMMAND_NAME_SIZE) return -1;
