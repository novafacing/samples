...
char *read_buf(int fd) {
  ...
  if (recv_size != sizeof(buf_size)) _terminate(1);
  ...
  if (buf_size > 0) {
    ...
    if (recv_size != buf_size) _terminate(4);
  }
...
void setup(void) {
  ...
  if (size != sizeof(write_fd)) _terminate(0);

  size = receive_all(read_fd, (char *)&read_fd, sizeof(read_fd));
  if (size != sizeof(read_fd)) _terminate(0);
...
void do_config(void) {
  ...
  if (size > MAX_PATTERN_SIZE) _terminate(3);
  ...
  while (pattern_offset < size) {
    while (current_offset > -1 &&
           pattern[pattern_offset] != pattern[current_offset])
...
int morris_pratt(char *buffer, int buffer_size) {
  ...
  while (buffer_offset < buffer_size) {
    while (pattern_offset > -1 &&
           pattern[pattern_offset] != buffer[buffer_offset])
