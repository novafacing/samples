...
size_t strlcat(char *dst, const char *src, const size_t dstsize) {
  ...
  return current_dst_len +
         strlcpy(dst + current_dst_len, src, dstsize - current_dst_len);
}
...
int transmit_str(int fd, char *s) { return transmit_all(fd, s, strlen(s)); }
