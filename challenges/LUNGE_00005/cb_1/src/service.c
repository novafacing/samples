...
void send_service_fd(int service_id, int new_stdin, int new_stdout) {
  printf(STDOUT, "Setting %s to read from %u and write to %u (via %u)\n",
         services[service_id].name, new_stdin, new_stdout,
         services[service_id].read_fd - 1);

  if (transmit_all(services[service_id].read_fd - 1, (char *)&new_stdout,
                   sizeof(new_stdout)) == 0)
    return;

  if (transmit_all(services[service_id].read_fd - 1, (char *)&new_stdin,
                   sizeof(new_stdin)) == 0)
    return;
}
...
int get_service_by_position(int position) {
  ...
  for (i = 0; services[i].name != NULL; i++) {
    if (services[i].configured && services[i].order == position) return i;
  }
...
int enable_services(int service_count) {
  ...
  for (i = 0; i < service_count; i++) {
    ...
    for (j = 0; j < service_count; j++) {
      if (services[j].configured == 0) printf(STDOUT, "%u", j);
    }
...
void setup_output(int enabled_count) {
  ...
  for (i = 0; i < enabled_count; i++) {
    ...
    if (current_service == -1) _terminate(5);
