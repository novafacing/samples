...
#define UNICHR(x)        \
  (PyUnicode_AS_UNICODE( \
      (PyUnicodeObject *)PyUnicode_DecodeASCII(x, strlen(x), NULL)));
...
static PyObject *escape_unicode(PyUnicodeObject *in) {
  ...
  out = (PyUnicodeObject *)PyUnicode_FromUnicode(
      NULL, PyUnicode_GET_SIZE(in) + delta);
  if (!out) return NULL;
  ...
  if (inp < inp_end)
    Py_UNICODE_COPY(outp, inp,
                    PyUnicode_GET_SIZE(in) - (inp - PyUnicode_AS_UNICODE(in)));
...
static PyObject *escape_silent(PyObject *self, PyObject *text) {
  if (text != Py_None) return escape(self, text);
...
static struct PyModuleDef module_definition = {PyModuleDef_HEAD_INIT,
                                               "markupsafe._speedups",
                                               NULL,
                                               -1,
                                               module_methods,
