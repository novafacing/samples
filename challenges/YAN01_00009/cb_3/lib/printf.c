...
void printf_setFlushOnNewline(int bEnabled) { gbFlushOnNewline = bEnabled; }
...
static size_t pflush(PrintfInfo* pInfo) {
  ...
  do {
    ret =
        transmit(pInfo->fd, pInfo->buf + total, pInfo->end - total, &rx_bytes);
    if ((ret != 0) || (rx_bytes == 0)) {
      return (-1);
    }
    total += rx_bytes;
...
static size_t uint32ToHexStr(char* str, uint32_t ui, int bLeadingZeroes,
                             int bUpcase) {
  if (str == NULL) {
    return (-1);
  }
    ...
    switch (fstr[i]) {
      case ('#'): {
        ...
        switch (fstr[i]) {
          ...
          case ('x'): {
            tempRet = uint32ToHexStr(pInfo->buf + pInfo->end,
                                     va_arg(args, uint32_t), bLeadingZeroes, 0);
            pInfo->end += tempRet;
            ret += tempRet;
            i++;
            break;
          ...
          case ('X'): {
            tempRet = uint32ToHexStr(pInfo->buf + pInfo->end,
                                     va_arg(args, uint64_t), bLeadingZeroes, 1);
            pInfo->end += tempRet;
            ret += tempRet;
            i++;
            break;
