...
StrFDMap gServers[MAX_SERVERS] = {
    {"walkthrough", 3, 3}, {"newsletter", 5, 5}, {"", 0, 0}};
...
typedef enum _E_States { INIT_STATE, CONNECTED_STATE, GOODBYE_STATE } E_States;
...
int findFDs(const char* server, int* pIn, int* pOut) {
  ...
  for (size_t i = 0; (i < MAX_SERVERS) && (gServers[i].server[0] != '\0');
       i++) {
    ...
    switch (state) {
      case (INIT_STATE): {
        if (strstr(buf, gBye) == buf) {
          ...
          printf("#s#s#s#s#s\n", gHelloReply1, name, gHelloReply2, uidStr,
                 gHelloReply3);
