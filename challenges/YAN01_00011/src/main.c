 ...
 *    1992 article in Dr. Dobbs
 *www.schneier.com/paper-pseudorandom-sequence.html As the article says, this is
 *not a very good generator. But it is good enough for our purposes Instead of
 *just returning the last bit, we return the whole uint32
 **/
static uint32_t gRandRegister = 0xDA2bAC9C;

uint32_t RANDOM() {
  gRandRegister =
      ((((gRandRegister >> 31) ^ (gRandRegister >> 6) ^ (gRandRegister >> 4) ^
         (gRandRegister >> 2) ^ (gRandRegister >> 1) ^ gRandRegister) &
        0x00000001)
       << 31) |
      (gRandRegister >> 1);
...
void srand(uint32_t seed) { gRandRegister = seed; }
...
int main(void) {
#define BUF_SIZE 64
#define READLINE(_buf, _len)     \
  do {                           \
    sret = readline(_buf, _len); \
    if (sret == 0) {             \
      _terminate(1);             \
    }                            \
  } while (0)
