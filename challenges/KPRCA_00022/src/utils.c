...
char *valid_eye_colors[] = {"Black", "Blue",   "Brown",  "Gray",
                            "Green", "Hazel",  "Maroon", "Multicolored",
                            "Pink",  "Unknown"};

char *valid_suffixes[] = {"Jr", "Sr",  "II",   "III", "IV", "V",
                          "VI", "VII", "VIII", "IX",  "X",  "Other"};

char *valid_states[] = {"AL", "AK", "AZ", "AR", "CA", "CO", "CT", "DE", "DC",
...
static int verify_choice(char **valid_choices, char *choice,
                         size_t num_elements) {
  size_t i;
...
int verify_hair(char *str, size_t size) {
  return verify_choice(valid_hair_colors, str,
                       sizeof(valid_hair_colors) / sizeof(char *));
}

int verify_eye(char *str, size_t size) {
  return verify_choice(valid_eye_colors, str,
                       sizeof(valid_eye_colors) / sizeof(char *));
}

int verify_suffix(char *str, size_t size) {
  return verify_choice(valid_suffixes, str,
                       sizeof(valid_suffixes) / sizeof(char *));
}

int verify_state(char *str, size_t size) {
  return verify_choice(valid_states, str,
                       sizeof(valid_states) / sizeof(char *));
}

int verify_education(char *str, size_t size) {
  return verify_choice(education_levels, str,
                       sizeof(education_levels) / sizeof(char *));
}
...
int verify_year(char *str, size_t size) {
  ...
  if (year < 1900 || year > 2014) return -1;
...
int verify_height_inches(char *str, size_t size) {
  ...
  if (inches < 0 || inches > 11) return -1;
...
int verify_weight(char *str, size_t size) {
  ...
  if (weight < 50 || weight > 500) return -1;
...
int verify_email(char *str, size_t size) {
  ...
  if (len > size || len < 3) return -1;
  len--;

  for (i = 0; i < len; i++) {
    char c = str[i];
    if (!isalnum(c) && c != '@' && c != '-' && c != '.' && c != '_') return -1;
    if (c == '@' && ++at_count == 2) return -1;
  }

  if (at_count != 1) return -1;
