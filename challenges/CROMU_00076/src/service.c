...
void initialize_map(pmap pm) {
  ...
  while ((pm->start_x == pm->end_x) || (pm->start_x - 1 == pm->end_x) ||
         (pm->start_x + 1 == pm->end_x) || (pm->start_y == pm->end_y) ||
         (pm->start_y + 1 == pm->end_y) || (pm->start_y - 1 == pm->end_y)) {
  ...
  while (success == 0) {
    ...
    switch (movement[0]) {
      case 'u':
        ...
        if (pm->current_y == 0) {
          printf("off map\n");
        } else if (pm->data[(pm->width * (pm->current_y - 1)) +
                            pm->current_x] == '#') {
          printf("blocked\n");
        } else {
      ...
      case 'l':
        if (pm->current_x == 0) {
          printf("off map\n");
        } else if (pm->data[(pm->width * (pm->current_y)) + pm->current_x -
                            1] == '#') {
          printf("blocked\n");
        } else {
          set_marker(pm->current_x, pm->current_y, pm, '\0');
