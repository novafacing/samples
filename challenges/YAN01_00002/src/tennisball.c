...
int snprintfloat(char* str, size_t len, float num) {
  ...
  if ((ret + 1) < len) {
    ...
    iNum = (uint32_t)((num - (float)iNum) *
                      (float)1000000.0);  // get the next 6 numbers
...
double strToDouble(const char* str) {
  ...
  while ((str[i] != '\0') && (str[i] != '\n')) {
    ...
    } else  // see if there are already too many digits
    {
      if (bWhole && (i >= 10)) {
        return (NAN.d);
      } else if (!bWhole && ((i - fracStart) >=
                             9))  // we can have an error here if we change 9 to
                                  // 10 - in that case we get whatever is in
  ...
  // something very curious - CLANG defaults to SSE instructions if available
  // and
  //  then uses x87 as a fallback. On the other hand, if we use long double
  //  instead of double then CLANG will use the x87 instructions instead. To
  //  keep things consistent, we might want to just use long double
}
 ...
 *
 **/
void fillPage(float v0_x, float v0_y, Coords* aData, int* pCount,
              int countsPerSecond) {
  ...
  float startTime =
      (float)*pCount /
      (float)countsPerSecond;  // this is the starting time in seconds
#endif
  int base = 0;
  int i = 0;
  int j = 0;
