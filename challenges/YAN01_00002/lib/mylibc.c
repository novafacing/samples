...
ssize_t readLine(int fd, char* buf, size_t len) {
  ...
  do {
    ...
    if (ret != 0) {
      // according to the documentation we can have 4 errors, EBADF (which is
      // what we are looking for)
      //  EINVAL, EFAULT should not be possible since they have to do with
      //  incorrect parameters and ENOMEM which is possible. At any rate, if
      //  this happens, we will just return errno
      return (-ret);
    }
