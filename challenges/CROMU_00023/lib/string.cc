...
size_t String::length(void) { return (m_length); }

bool String::empty(void) { return (m_length == 0); }
...
String String::SubString(size_t startPos, size_t endPos) const {
  if (startPos >= m_length) return String("");

  if (endPos > m_length) endPos = m_length;

  if (startPos >= endPos) return String("");
...
bool String::operator==(const String &rhs) const {
  ...
  for (size_t i = 0; i < m_length; i++)
    if (m_pData[i] != rhs.m_pData[i]) return (false);
