...
char gHelpMsg[] =
    ...
    "010b : PUSHPC : Pushes the program counter of the current instruction "
    "(i.e. PUSHPC) onto the stack\n"
    "011b : JMPZ : Pops the top number off of the stack, if its 0 then pop the "
    "next value off of the stack and store t\n"
    "hat into the PC. If it is not-zero then just pop the next value off of "
    "the stack and do nothing with it.\n"
    "100b : SWAP # : Swaps the nth number with the top one (which is 0) - "
    "meaning SWAP 0 does nothing\n"
    "101b : DUP #: Duplicates nth number and pushes it to the top of the "
    "stack\n"
    "110b : ADD : Pops the top two numbers off of the stack, adds them, and "
    "pushes the sum back onto the stack\n"
    "111b : SUB : Pops the top two numbers off of the stack, subtracts them ( "
    "(top - 1) - top ), and pushes the differ\n"
    "ence back onto the stack\n"
    "\n"
    "All instructions are 32-bits long with the first 3 bits (msbs) reserved "
    "for the instruction and the lower 29 bits\n"
    "for literals. POP, ADD, SUB, PUSHPC, JMPZ ignores the lower 29 bits, "
    "UNLESS the following special instructions a\n"
    "re used:\n"
    "\n"
    "0xFFFFFFFF : RET : End of instruction stream - The topmost value is the "
    "return value\n"
    "\n"
    "### A simple program\n"
    "\n"
    ...
    "RET\n"
    "\n"
    "If y-1 is not zero, then we will jump back up to where PUSHPC is and "
    "execute that whole stream again, except SUM \n"
    "will now be SUM+x and y is now y-1\n"
    "If y-1 is zero, then we will jump to the instruction after the second "
    "JMPZ (which is the first POP)\n";
...
int main(void) {
  ...
  do {
    total = 0;
    do {
      ret = receive(STDIN, ((char*)(&insn)) + total, sizeof(insn) - total,
                    &rx_bytes);
      if ((ret != 0) || (rx_bytes == 0)) {
        if (totalInsns == 0) {
    ...
    if (totalInsns >= _MAX_INSNS_SIZE_) {
      transmit_all(_INSNS_OVERFLOW_EXCP_MSG_,
                   sizeof(_INSNS_OVERFLOW_EXCP_MSG_));
      _terminate(-1);
    }
  } while (insn != END_INSN);
    ...
    switch (INSN_TO_OPCODE(insn)) {
      case (PUSH): {
        ...
        if (curTop >= _MAX_STACK_SIZE_) {
          transmit_all(_STACK_OVERFLOW_EXCP_MSG_,
                       sizeof(_STACK_OVERFLOW_EXCP_MSG_));
          _terminate(-1);
        }
#endif
      ...
      case (POP): {
        if (curTop < 0) {
          transmit_all(_STACK_UNDERFLOW_EXCP_MSG_,
                       sizeof(_STACK_UNDERFLOW_EXCP_MSG_));
          _terminate(-1);
        }
        curTop--;
        break;
