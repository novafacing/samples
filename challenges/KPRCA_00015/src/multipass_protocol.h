...
** Operations **
 ...
 - REFUND: multi-pass is used to cancel a purchase and refund the payment
        amount to the card's balance. The original purchase transaction is
removed from the server's transaction log.
 ...
 * Packet Sequence Overview *
 ...
 the card_id / auth_code are on record and if so, to start a new transaction log
entry and mark the transaction as 'AUTH'.
 2. The second packet is of type OPS. If a previous AUTH was completed for the
 given transaction ID and card (ID/auth_code),
 the server uses this packet to perform the given operation and mark the
transaction as operation 'OPS'.
 ...
 1. The first packet is of type INIT. The server uses this packet to create a
new card using the next card id, the next auth_code, and the client-provided
inital balance. It also starts a new transaction log marked as 'INIT'.
...
** Communications - client<->server **
        ...
        * Vendor Data *
                ...
                uint8_t vendor_location_sz; char[vendor_location_sz]
vendor_location;

 * ALL operations *
 server -> client (PKT_TYPE = INIT or AUTH or OPS or FIN)
 - IFF Common Packet Header status is non-zero, server sends Error Data
 -- msg is the error message associated with status; does not contain a trailing
'\0'
 ...
 * PURCHASE operation *
 ...
 -- card_id and auth_code will be the same as those provided to the server by
the client. client -> server (PKT_TYPE = OPS)
 - client sends Common Packet Header followed by Purchase Data and associated
Vendor Data
 ...
 -- card_id and auth_code will be the same as those provided to the server by
the client. client -> server (PKT_TYPE = FIN)
 - client sends Common Packet Header
 ...
 * RECHARGE operation *
 ...
 -- card_id and auth_code will be the same as those provided to the server by
the client. client -> server (PKT_TYPE = OPS)
 - client sends Common Packet Header followed by Recharge Data and associated
Vendor Data
 -- card_id and auth_code must be from the card used during AUTH.
 -- transaction_id must be that provided by server during AUTH
 -- status is unused (recommend to set to 0)
...
the client. client -> server (PKT_TYPE = OPS)
 ...
 -- transaction_id must be that provided by server during AUTH
 -- status is unused (recommend to set to 0)
 -- count tells the server to send up to that many of the most recent
transactions that match the given card. server -> client (PKT_TYPE = OPS)
...
the client.
 ...
 --- if transaction OP_CODE==PURCHASE, send Purchase Data and associated Vendor
Data
 --- if transaction OP_CODE==BALANCE, send Balance Data
 --- if transaction OP_CODE==ISSUE, send Issue Data
 --- if transaction OP_CODE==RECHARGE, send Recharge Data and associated Vendor
Data
...
the client. client -> server (PKT_TYPE = OPS)
 ...
 -- transaction_id must be that provided by server during AUTH
 -- status is unused (recommend to set to 0)
 -- purchase_id is the purchase_id from a previous purchase with this associated
card.
 -- Purchase Data's transaction_id is the transaction_id from the previous
purchase associated with purchase_id. server -> client (PKT_TYPE = OPS)
 - server sends Common Packet Header
 -- status will be set to a value from the STATUS enum.
 -- transaction_id will be set to the ID of the current transaction.
 -- card_id and auth_code will be the same as those provided to the server by
 ...
 3. Then the server will exit.
        ...
        * Error status codes *
        ...
        typedef enum {
                ...
                ERRNO_MP_RECHARGE_FULL,	// Recharge would case card balance to
wrap ERRNO_MP_REFUND_FULL,	// Refund would cause card balance to wrap
                ERRNO_MP_NO_HISTORY,	// No matching transaction history in
transaction list. ERRNO_MP_INVALID_OPCODE,	// Invalid Op_code
                ERRNO_MP_INVALID_P_TYPE,	// Invalid Packet Type
                ERRNO_MP_INVALID_V_TYPE,	// Invalid Vendor Type
                ERRNO_MP_NO_AUTH,		// Transaction did not complete
AUTH step ERRNO_MP_NO_INIT,		// Transaction did not complete INIT
step ERRNO_MP_NO_OPS,		// Transaction did not complete OPS step
                ERRNO_MP_NOT_FOUND,		// When the card or transaction
is not in the list ERRNO_MP_INVALID_CARD,	// Card does not exist or is
invalid ERRNO_MP_RECV,			// Failed to receive expected number of
bytes ERRNO_MP_UNK			// Unknown error } STATUS;
