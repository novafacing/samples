...
void *malloc(size_t size) {
  ...
  if (state.initialised == 0) malloc_init();

  if (size > state.largest_bucket) return malloc_large(size);

  for (i = 0; i < MAX_BUCKETS; i++) {
    if (size <= state.buckets[i].alloc_limit) break;
  }
  ...
  ptr = (void *)(((char *)chunk) + sizeof(chunk_header_t) +
                 (bucket->alloc_limit * i));
  return ptr;
}
