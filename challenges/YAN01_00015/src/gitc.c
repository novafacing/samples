...
/*
#define NUM_SKIPS 5
TextProtectSkip skips[NUM_SKIPS] = { {0x80486c8,5}, {0x80488ab,5},
{0x80488c6,7}, {0x804910c,10}, {0x8048210,7} };
*/

#define DATA_START (0x804C000)
#define DATA_SIZE (PAGE_SIZE * 0x18)

// We have to be careful with the TEXT hash because the value that we are
// checking against will
//  also be included in the text section which will change the value of the hash
//  so the best way to handle this is to ignore the text protect function ..
...
void textProtect() {
  ...
  for (; (uint32_t)text < (TEXT_START + TEXT_SIZE); text += 1) {
    ...
    for (i = 0; i < NUM_SKIPS; i++) {
      // We need to check the lower address -4 since we are reading 4 bytes at a
      // time.
      if (((uint32_t)text >= (skips[i].addr - 4)) &&
          ((uint32_t)text <= (skips[i].addr + skips[i].size))) {
        bHash = 0;
        break;
      }
    }
...
#ifdef PATCHED_2
  if ((((uint32_t)addr & PAGE_MASK) == 0) ||
      (((uint32_t)addr & PAGE_MASK) == (MAGIC_PAGE & PAGE_MASK)))
#else
  if ((((uint32_t)addr & PAGE_MASK) == 0) ||
      (((uint32_t)addr & PAGE_MASK) == (0 & PAGE_MASK)))
#endif
  {
    return (0);
  }
...
char* errorMsgs[] = {"Huh? I don't understand. Come again?",
                     "You are still breaking up.", "Alright fine. I give up."};
...
void printLights(void* addr) {
  ...
  if (isValidAddr(addr)) {
    ...
    // shift of 0 is undefined so we have to be careful, that is why we are
    // looping
    //  up to 7 only
    for (i = 0; i < 7; i++) {
      if ((contents >> (7 - i)) & 0x1) {
