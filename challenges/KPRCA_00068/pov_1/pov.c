...
static char pkt[] =
    "\x64\x75\x61\x2e\x18\x00\x00\x00\x40\x00\x00\x00\x00\x00\x00\x00\x40\x1f"
    "\x00\x00\x01\x00\x00\x00\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF";
...
int main() {
  ...
  if (*(unsigned int *)buf >= 0x40) {
    ...
    value[6] = buf[0x32];
    value[7] = buf[0x33];
    for (i = 7; i > 0; i--) value[i] ^= (value[i - 1] >> 1) ^ 0x55;
  }
