...
int receive_all(int fd, void *buf, size_t count, size_t *rx_bytes) {
  ...
  while (bytes_left) {
    ...
    if (SUCCESS != (ret = receive(fd, buf + (count - bytes_left), bytes_left,
                                  &rx_bytes_local))) {
#ifdef DEBUG
...
int pkt_send(size_t tid, char src, char msg) {
  ...
  if (SUCCESS != (ret = pkt_serialize(&pkt, src, msg))) {
    return ret;
  }
  ...
  if (SUCCESS != (ret = transmit_all(fd, &pkt, sizeof(pkt), NULL))) {
    return ret;
  }
...
void pkt_dump(struct packet *pkt) {
  ...
  // msg doesn't need to be validated because the str_syscall[] array has 256
  // entries.
...
struct list *list_create() {
  ...
  if (SUCCESS != (allocate(PAGE_SZ, 0, (void *)&new))) {
    return NULL;
  }
  new->head = NULL;
  new->tail = NULL;
...
struct node *list_remove(struct list *lst, char value) {
  ...
  if (lst->head == needle) {
    lst->head = needle->next;
  }

  // if we're removing the last element...
  if (lst->tail == needle) {
    lst->tail = needle->prev;
  }
