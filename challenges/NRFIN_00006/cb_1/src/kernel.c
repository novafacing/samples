...
int syscall_futex(unsigned char syscall, char **curr, char *end) {
  ...
  if (tid < 1 || tid > NUM_THREADS) {
    ...
    fprintf(stderr, "%s: [E] %s: invalid tid: 0x%02x\n", STR_THREAD_NAME,
            str_syscall[syscall], syscall_minor);
#endif
  ...
  for (int i = 0; i < ACK_TRIES; i++) {
    ...
    if (SUCCESS != (ret = fdwait(PIPE_RECV + 1, &fdsToWait, NULL, &timeToWait,
                                 &fdsReady))) {
#ifdef DEBUG
  ...
  if (got_resp &&
      (SUCCESS == receive_all(PIPE_RECV, &pkt, sizeof(pkt), NULL))) {
    unsigned char syscall = pkt_get_msg(&pkt);
    unsigned char src = pkt_get_src(&pkt);

    if (src < 1 || src > NUM_THREADS) {
#ifdef DEBUG
      fprintf(stderr, "%s: [E] received ACK from invalid TID: 0x%02x",
              STR_THREAD_NAME, src);
#endif
    ...
    if ((syscall & MASK_ACK)) {
      switch (syscall_minor) {
        case MINOR_FUTEX_WAIT:
          if ((MAJOR_FUTEX | MINOR_FUTEX_WAIT | MASK_PENDING) ==
              thread_state[src]) {
            struct node *new = NULL;
            if (NULL == (new = node_create(src))) {
#ifdef DEBUG
              fprintf(stderr,
                      "%s: [E] futex(%s): node_create(): failed allocation\n",
                      STR_THREAD_NAME, str_syscall[syscall]);
#endif
            ...
            thread_state[src] = MAJOR_UNKNOWN;
#ifdef DEBUG
            fprintf(stderr, "%s: [E] futex(%s): unexpected response: 0x%02x",
        ...
        case MINOR_FUTEX_WAKE:
          if ((MAJOR_FUTEX | MINOR_FUTEX_WAKE | MASK_PENDING) ==
              thread_state[src]) {
...
#ifdef DEBUG
              fprintf(stderr,
                      "%s: [D] futex(%s): list_remove: attempted to remove a "
                      "non-element\n",
                      STR_THREAD_NAME, str_syscall[syscall]);
#endif
            } else {
...
#ifdef DEBUG
                fprintf(stderr,
                        "%s: [E] futex(%s): error during node_destroy()\n",
                        STR_THREAD_NAME, str_syscall[syscall]);
#endif
                thread_state[src] = MAJOR_UNKNOWN;
              }
        ...
        case MINOR_FUTEX_REQUEUE_PI:
          if ((MAJOR_FUTEX | MINOR_FUTEX_REQUEUE_PI | MASK_PENDING) ==
              thread_state[src]) {
        ...
        case MINOR_FUTEX_CMP_REQUEUE_PI:
          if ((MAJOR_FUTEX | MINOR_FUTEX_CMP_REQUEUE_PI | MASK_PENDING) ==
              thread_state[src]) {
            ...
            if (op == (~src & 0xFF)) {
              ...
              if (NULL == list_remove(pi_queue, src)) {
                ...
                fprintf(
                    stderr,
                    "%s: [E] futex(%s): failed list_remove() (actual error)\n",
                    STR_THREAD_NAME, str_syscall[syscall]);
#endif
                thread_state[src] = MAJOR_UNKNOWN;
              }
        ...
        default:
#ifdef DEBUG
          fprintf(stderr, "%s: [E] invalid response: 0x%02x", STR_THREAD_NAME,
                  syscall);
#endif
          ret = ERRNO_RECV;
          break;
...
#ifdef DEBUG
      fprintf(stderr, "%s: [D] %s is now: %s\n", STR_THREAD_NAME, str_tids[src],
              str_syscall[thread_state[src]]);
#endif

    } else {
#ifdef DEBUG
      fprintf(stderr, "%s: [E] got a response that wasnt an ACK: 0x%02x (%s)",
              STR_THREAD_NAME, syscall, str_syscall[syscall]);
#endif
      ret = ERRNO_INVALID_ACK;
    }
  } else {
...
// PAUSE takes 2 bytes: 1 to determine action, 1 to provide argument to that
// action. Functionality wholly contained within KERNEL; requires no ACK from
// threads. Requires no validation of arg because sleeps between 0 and 255s are
// legit.
...
//  arg1: the "message" we wish to send (same message is sent on each recursion
//  to make it simpler)
  ...
  switch (syscall_minor) {
    case MINOR_PROC_THREAD_STATE:
      ...
      for (int i = 1; i <= NUM_THREADS; i++) {
        ...
        fprintf(stderr,
                "%s: [D] syscall_proc(): thread_state[%d] = 0x%02x (%s)\n",
      ...
      // TODO: Why is transmit() returning EINVAL? (according to
      // /usr/include/libcgc.h)
      if (SUCCESS !=
              (ret = transmit_all(STDOUT, thread_state + 1, len, &tx_bytes)) ||
          tx_bytes != len) {
#ifdef DEBUG
        fprintf(stderr,
                "%s: [E] proc(): error during thread_state transmit(); ret = "
                "0x%02x; len = 0x%02x\n",
                STR_THREAD_NAME, ret, len);
        fprintf(stderr,
                "%s: [E] proc(): thread_state + 1: %d; thread_state[1]: %d; "
                "&(thread_state[1]): %d; start = %d\n",
                STR_THREAD_NAME, thread_state + 1, thread_state[1],
                &(thread_state[1]), start);
#endif
      }
      break;
    ...
    case MINOR_PROC_LIST_DUMP:
      ...
      if (SUCCESS != (ret = transmit_all(STDOUT, pi_queue, len, &tx_bytes)) ||
          tx_bytes != len) {
#ifdef DEBUG
        fprintf(stderr, "%s: [E] proc(): error during pi_queue transmit()\n",
                STR_THREAD_NAME);
#endif
      } else {
        ...
        while (NULL != curr) {
          if (SUCCESS != (ret = transmit_all(STDOUT, curr, len, &tx_bytes)) ||
              tx_bytes != len) {
#ifdef DEBUG
            fprintf(stderr, "%s: [E] proc(): error during node transmit()\n",
                    STR_THREAD_NAME);
#endif
            break;
          }
...
int syscall_tgkill(unsigned char syscall, char **curr, char *end) {
  ...
  unsigned char syscall_minor = syscall & MASK_MINOR;
  if (0 != syscall_minor || syscall & MASK_ACK || syscall & MASK_PENDING) {
#ifdef DEBUG
  ...
  if (got_resp &&
    ...
    if (src < 1 || src > NUM_THREADS) {
      ...
      if ((MAJOR_TGKILL | MASK_ACK) == (syscall & (MASK_MAJOR | MASK_ACK)) &&
          (MAJOR_TGKILL | MASK_PENDING) ==
              (thread_state[src] & (MASK_MAJOR | MASK_PENDING))) {
        ...
        thread_state[src] = MAJOR_UNKNOWN;
#ifdef DEBUG
        fprintf(stderr, "%s: [E] unexpected response: 0x%02x", STR_THREAD_NAME,
                str_syscall[syscall], syscall);
#endif
      }
    }
...
int main() {
  ...
  while (1) {
    ...
    size_t rx_bytes = 0;
    if (SUCCESS != (ret = receive_all(STDIN, buf, 1, &rx_bytes)) ||
        rx_bytes != 1) {
#ifdef DEBUG
    ...
    // TODO: Why is transmit() returning EINVAL? (according to
    // /usr/include/libcgc.h)
    if (SUCCESS != (ret = receive_all(STDIN, buf, len, &rx_bytes)) ||
        rx_bytes != len) {
#ifdef DEBUG
      fprintf(stderr,
              "%s: [E] wrong number of syscall bytes read; bailing...\n",
              STR_THREAD_NAME);
#endif
      goto _bail_main;
    }
    ...
    // start the parsing
    if (ERRNO_SHUTDOWN == syscall_dispatch(&curr, buf + len)) {
      goto _bail_main;
    }
  }
