...
int xlat_delimited_read(int fd, unsigned char **buf, unsigned int *size,
                        unsigned char *delim, unsigned int delim_len) {
  unsigned char tmp[delim_len];
  unsigned int i;
  int retval;

  if (xlat_init) {
    for (i = 0; i < delim_len; i++) tmp[i] = xlat_map[delim[i]];
    retval = delimited_read(fd, buf, size, tmp, delim_len);
    for (i = 0; i < *size; i++) (*buf)[i] = xlat_map_inv[(*buf)[i]];
    return retval;
  } else {
...
int xlat_length_read(int fd, unsigned char *buf, unsigned int len) {
  ...
  if (xlat_init) {
    ...
    for (i = 0; i < len; i++) buf[i] = xlat_map_inv[buf[i]];
    return retval;
  } else {
...
int xlat_transmit_all(int fd, const void *buf, const size_t size) {
  ...
  for (i = 0; i < size;) {
    ...
    for (unsigned int j = 0; j < cnt; j++) tmp[j] = xlat_map[tmp[j]];
