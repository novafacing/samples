...
#include "libxdr.h"
int marshal_result_status(result_status *data, unsigned char **_out,
                          size_t *_size) {
  int _result;
...
int unmarshal_result_status(result_status *data, unsigned char **_in,
                            size_t *_size) {
  int _result;
...
int unmarshal_key(key *data, unsigned char **_in, size_t *_size) {
  ...
  CHECK_RESULT();
  if ((*data).data.count > 200) return -1;
  _result = _checked_calloc((void **)&(*data).data.data, (*data).data.count,
                            sizeof((*data).data.data[0]));
  {
    size_t i;
    for (i = 0; i < (*data).data.count; i++) {
...
int unmarshal_result(result *data, unsigned char **_in, size_t *_size) {
  ...
  CHECK_RESULT();
  if ((*data).rec.count > 1) return -1;
  _result = _checked_calloc((void **)&(*data).rec.data, (*data).rec.count,
                            sizeof((*data).rec.data[0]));
  {
    size_t i;
    for (i = 0; i < (*data).rec.count; i++) {
