...
static int _checked_calloc(void **ptr, unsigned int count, size_t size) {
  ...
  if ((unsigned long long)count * size > INT32_MAX) return -1;
...
static int marshal_unsigned_int(unsigned long *data, unsigned char **out,
                                size_t *size) {
  unsigned char *_out = *out;
...
static int marshal_opaque(unsigned char *data, unsigned char **out,
                          size_t *size) {
  if (*size < 1) return -1;
...
static int unmarshal_unsigned_int(unsigned long *data, unsigned char **in,
                                  size_t *size) {
  unsigned char *_in = *in;
...
static int unmarshal_opaque(unsigned char *data, unsigned char **in,
                            size_t *size) {
  if (*size < 1) return -1;
