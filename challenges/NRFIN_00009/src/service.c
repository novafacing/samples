...
char* setValue(char* buffer, char* value) {
  ...
  ptr = buffer;
  while (*ptr && (*ptr != *delim) && count < RESULT_VALUE_SIZE) ptr++, count++;
...
int parseResultSize(char* buffer) {
  ...
  count = 0;
  while (*end && (*end != *delim) && count < KEY_SIZE) end++, count++;
...
int parseBalanceResult(char* buffer) {
  ...
  if (strcmp(key, BAL_STR) == 0) {
    ...
    if (balance < 0 || balance > 255) return 0;
    return balance;
  }
...
int receiveNumResults(int socket) {
  ...
  if (num_results > 0) return num_results;
...
unsigned int getRandomNumber(unsigned int max) {
  ...
  if (bytes_written != sizeof(index)) _terminate(2);
...
int purchaseSong(int socket, Song* selectedSong, SongList* mySongList) {
  ...
  if (price < 0) {
    ret =
        transmit_all(socket, INVALID_PRICE_MSG, sizeof(INVALID_PRICE_MSG) - 1);
    if (ret != 0) _terminate(10);
  ...
  if (mySongList->balance >= price) {
    ...
    mySongList->size++;
    memcpy(&mySongList->songs[mySongList->size - 1], selectedSong,
           RESULT_VALUE_SIZE * 3);

    ret = transmit_all(socket, mySongList->songs[mySongList->size - 1].id,
                       strlen(mySongList->songs[mySongList->size - 1].id));
    if (ret != 0) _terminate(10);
...
#else
#endif
  ret = allocate(sizeof(Song) * mySongList->balance, 0,
                 (void**)&mySongList->songs);
  if (ret != 0) _terminate(3);
