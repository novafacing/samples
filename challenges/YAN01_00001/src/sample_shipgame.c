...
static const char CMD_PLACE = 'P';  // Prcd1 -- 1 is the ship number, r is Row,
                                    // C is Column, d is direction (Up or Down)
...
const char* GAME_ALREADY_STARTED_MSG =
    "Game already started - try to hit something\n";
...
int processPlace(PlayerState* ps, Command* cmd) {
  ...
  if (cmd->shipNum == SHIP2) {
    ...
    if (cmd->dir == DIR_R) {
      if ((ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL) ||
          (ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col) + 1] !=
           EMPTY_CELL)) {
        return (-1);
      }
      ...
      if ((ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL) ||
          (ps->board[ROW_TO_NUM(cmd->row) + 1][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL)) {
        return (-1);
      }
    ...
    if (cmd->dir == DIR_R) {
      if ((ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] !=
          ...
          (ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col) + 1] !=
           EMPTY_CELL) ||
          (ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col) + 2] !=
           EMPTY_CELL)) {
        return (-1);
      }
      ...
      if ((ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] !=
          ...
          (ps->board[ROW_TO_NUM(cmd->row) + 1][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL) ||
          (ps->board[ROW_TO_NUM(cmd->row) + 2][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL)) {
        return (-1);
      }
    ...
    if (cmd->dir == DIR_R) {
      if ((ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] !=
          ...
          (ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col) + 2] !=
           EMPTY_CELL) ||
          (ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col) + 3] !=
           EMPTY_CELL)) {
        return (-1);
      }
      ...
      if ((ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] !=
          ...
          (ps->board[ROW_TO_NUM(cmd->row) + 2][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL) ||
          (ps->board[ROW_TO_NUM(cmd->row) + 3][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL)) {
        return (-1);
      }
    ...
    if (cmd->dir == DIR_R) {
      if ((ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] !=
          ...
          (ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col) + 3] !=
           EMPTY_CELL) ||
          (ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col) + 4] !=
           EMPTY_CELL)) {
        return (-1);
      }
      ...
      if ((ps->board[ROW_TO_NUM(cmd->row)][COL_TO_NUM(cmd->col)] !=
          ...
          (ps->board[ROW_TO_NUM(cmd->row) + 3][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL) ||
          (ps->board[ROW_TO_NUM(cmd->row) + 4][COL_TO_NUM(cmd->col)] !=
           EMPTY_CELL)) {
        return (-1);
      }
...
static inline int allShipsSunk(PlayerState* ps) {
  ...
  return ((ps->ship2Life == 0) && (ps->ship3Life == 0) &&
          (ps->ship4Life == 0) && (ps->ship5Life == 0));
}
  ...
  while (1) {
    ...
    if (ret == 0)  // if its okay
    {
      if (cmd.cmdNum == CMD_EXIT) {
        ...
        if (bStarted) {
          write(STDOUT, GAME_ALREADY_STARTED_MSG,
                myStrLen(GAME_ALREADY_STARTED_MSG));
        } else {
          bStarted = 1;
        }
