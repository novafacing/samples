...
void largeIntMul(ulargeint_t* product, ulargeint_t* op1, ulargeint_t* op2) {
  ...
  for (i = 0; i < LARGE_INT_BITS; i++) {
    if (op2->bytes[LARGE_INT_ARR_LEN - 1 - (i / (BITS_IN_UINT32))] &
        (0x1 << (i % (BITS_IN_UINT32)))) {
...
handler_func_t handlerTable[HANDLER_TABLE_SIZE][HANDLER_TABLE_SIZE]
                           [HANDLER_TABLE_SIZE];

void userMenu(uint32_t u);

void userMenu1() { userMenu(1); }

void userMenu2() { userMenu(2); }

void userMenu3() { userMenu(3); }

void userMenu4() { userMenu(4); }

void userMenu5() { userMenu(5); }

void userMenu6() { userMenu(6); }

void userMenu7() { userMenu(7); }

void userMenu8() { userMenu(8); }

void userMenu9() { userMenu(9); }

void userMenu10() { userMenu(10); }

void userMenu11() { userMenu(11); }

void userMenu12() { userMenu(12); }

void userMenu13() { userMenu(13); }

void userMenu14() { userMenu(14); }

void userMenu15() { userMenu(15); }

void userMenu16() { userMenu(16); }

void userMenu17() { userMenu(17); }

void userMenu18() { userMenu(18); }

void userMenu19() { userMenu(19); }

void userMenu20() { userMenu(20); }

void userMenu21() { userMenu(21); }
...
int userToPassword[MAX_USERS] = {6,  9,  12, 18, 19, 20, 24, 25, 27, 28, 29,
                                 30, 36, 38, 40, 41, 42, 44, 45, 46, 48};
...
int addPW(uint32_t u, uint32_t x, uint32_t y, uint32_t z) {
  ...
  for (i = 0; i < MAX_PASSWORDS; i++) {
    if ((userPasswords[u][i][0] == 0) && (userPasswords[u][i][1] == 0) &&
...
int checkLogin(ulargeint_t* pUserNum, ulargeint_t* px, ulargeint_t* py,
               ulargeint_t* pz) {
  ulargeint_t tempSum;
  ulargeint_t tempProduct;

#ifdef PATCHED
  if ((largeIntCmp(px, py) >= 0) || (largeIntCmp(py, pz) >= 0) ||
