...
static size_t mystrndup(const char *str, size_t n, char **out) {
  ...
  if (n == 0 || (result = malloc(n + 1)) == NULL) return 0;
...
static uint32 recv_bytes(server_t *server, uint32 count) {
  ...
  if (count & 7) count = (count + 7) & ~7;

  if ((count + server->packet_len) > PACKET_SIZE) return 0;
  if (count > PACKET_SIZE) return 0;
...
static int get_byte(server_t *server, byte *out) {
  if (server->packet_idx + 1 > server->packet_len) return 0;
...
static int get_uint32(server_t *server, uint32 *out) {
  ...
  if (server->packet_idx + 4 > server->packet_len) return 0;
...
static int get_uint64(server_t *server, uint64 *out) {
  ...
  if (server->packet_idx + 8 > server->packet_len) return 0;
...
static int write_byte(server_t *server, byte b) {
  if (server->packet_len + 1 > PACKET_SIZE) return 0;
...
static int write_uint32(server_t *server, uint32 x) {
  if (server->packet_len + 4 > PACKET_SIZE) return 0;
...
static int write_uint64(server_t *server, uint64 x) {
  if (server->packet_len + 8 > PACKET_SIZE) return 0;
...
static int write_string(server_t *server, const char *s, uint32 length) {
  if (!write_uint32(server, length)) return 0;

  if (server->packet_len + length > PACKET_SIZE) return 0;
...
static int get_string(server_t *server, char *out, size_t n) {
  ...
  if (!get_uint32(server, &length)) return 0;

  if (length + 1 > n) return 0;

  if (server->packet_idx + length > server->packet_len) return 0;

#ifdef PATCHED_1
  if (length > server->packet_len) return 0;
#endif
...
static uint32 recv_packet(server_t *server) {
  ...
  if (!recv_bytes(server, 8)) return 0;

  if (!get_uint32(server, &packet_len)) return 0;
  if (!get_byte(server, &padding_len)) return 0;

  if (packet_len < 1 || padding_len + 1 > packet_len) return 0;

  if (packet_len > 4 && !recv_bytes(server, packet_len - 4)) return 0;
...
static int send_packet(server_t *server) {
  ...
  if (server->packet_len > 32768) return 0;
...
static int userauth_password(server_t *server, const char *username,
  ...
  if (!get_boolean(server, &mode)) return 0;

  if (!get_string_alloc(server, &password)) goto done;

  if (mode && !get_string_alloc(server, &new_password)) goto done;
  ...
  success =
      strcmp(password, cred == NULL ? "fakepassword" : cred->password) == 0 &&
      cred != NULL;
...
static int userauth_handler(server_t *server, byte type) {
  ...
  if (type != MSG_USERAUTH_REQUEST) return send_unrecognized(server);

  if (!get_string_alloc(server, &username)) goto done;

  if (!get_string_alloc(server, &service_name)) goto done;

  if (!get_string_alloc(server, &auth_name)) goto done;
...
static int handle_service_request(server_t *server) {
  ...
  if (!get_string(server, service_name, sizeof(service_name))) return 0;
...
int main() {
  ...
  while (1) {
    ...
    if (!get_byte(&server, &msg)) break;
    if (msg == MSG_DISCONNECT) {
      break;
    } else if (msg == MSG_IGNORE) {
      continue;
    } else if (msg == MSG_SERVICE_REQUEST) {
      if (!handle_service_request(&server)) break;
    } else if (server.service_handler) {
      if (!server.service_handler(&server, msg)) break;
    } else {
