...
store_tree_t *make_tree(store_tree_t *l, store_tree_t *r, char *key,
                        char *val) {
  if (!key || !val) return ((void *)0);
...
unsigned char *read_command(int fd) {
  ...
  if (read_n_bytes(fd, sizeof(size), (char *)&size) < 0) return ((void *)0);

  if (strncmp("quit", (char *)&size, 4) == 0) exit(0);

  if (size > (16 * 1024) || size < 0) return ((void *)0);
...
char *handle_append(command_t *command) {
  ...
  if (!node) {
    ins_tree(groot, key, command->argv[1]), node = find_tree(groot, key);
  } else {
...
char *handle_bitcount(command_t *command) {
  ...
  if (strlen(p) > 0) {
    int i = strlen(p);
    unsigned mask = 0;
    for (int ii = 0; ii < i; ii++) mask |= (0xFF << (ii * 8));
...
char *handle_bitop(command_t *command) {
  enum bop_type { AND, OR, XOR, NOT } bop;
  ...
  switch (bop) {
    ...
    case XOR:
      ...
      break;
    case NOT:
      for (size_t i = 0; i < llen; i++) res[i] = ~safe_str_index(s2, i, 0);
      break;
  }
...
char *handle_keys(command_t *command) {
  ...
  if (strlen(buffer) == 0) return "empty";
  return buffer;
}
...
char *handle_rename(command_t *command) {
  ...
  if (!src) return (char *)_false;

  char *v = src->val;
  if (src == dst) return (char *)_false;
...
int main(void) {
  ...
  for (;;) {
    ...
    if (!serialized_command) continue;
    ...
    for (i = 0; i < 12; i++)
      if (commands[i].type == command->type &&
          commands[i].arity == command->arity)
        break;
    ...
    switch (command->type) {
      ...
      case CMD_INCR:
      case CMD_KEYS:
        if (result == synerr || result == _true || result == _false ||
            result == nil)
          break;
        else
          free(result);
