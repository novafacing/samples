...
#define ERRQUIT "Mkay"
#define SENDM(b, s) \
  if (sendmsg(STDOUT, b, s) < 0) _terminate(23);
#define RECVM(b, s, ss)               \
  if (recvmsg(STDIN, b, s, ss) < 0) { \
    create_resp_pkt(curpkt, ERRQUIT); \
    SENDD(curpkt, curpkt->hdr.size);  \
    _terminate(24);                   \
  }
#define RECVD()                       \
  if (recvdgram(STDIN) < 0) {         \
    create_resp_pkt(curpkt, ERRQUIT); \
    SENDD(curpkt, curpkt->hdr.size);  \
    _terminate(25);                   \
  }
#define SENDD(m, s) \
  if (senddgram(STDOUT, m, s) < 0) _terminate(26);
#define MAX_FRAG 256
...
enum pkttype { PUT, GET, RAND, RESP, FRAG, UNKNOWN };
